...
public class Sim_chisquare_obj implements ContinuousGenerator {
   ...
   */
  public Sim_chisquare_obj(String name, long deg_freedom) {
    if (deg_freedom <= 0L) {
      throw new Sim_parameter_exception("Sim_chisquare_obj: The degrees of freedom must be a positive integer.");
    }
   ...
   */
  public Sim_chisquare_obj(String name, long deg_freedom, long seed) {
    if (deg_freedom <= 0L) {
      throw new Sim_parameter_exception("Sim_chisquare_obj: The degrees of freedom must be a positive integer.");
    }
  ...
  public double sample() {
    ...
    for (long i=0L; i < deg_freedom; i++) {
      result += Sim_normal_obj.sample(source, 0.0, 1.0);
    }
    return result;
  }

  // Used by other distributions that rely on the Chi-Square distribution
