...
*/
public class JobsToUsers extends javax.swing.JFrame {
	/**
	 * 
	 */
	...
	private AbstractAction refreshClick;
	...
	*/
	...
	private void initGUI() {
		try {
			...
			}
			{
				...
				lblAvailable.setText("Available Jobs:");
			}
			{
			...
			}
			{
				...
				lblApplied.setText("Jobs for this user:");
			}
			{
				lblWhich = new JLabel();
				lblWhich.setText("Which User to Add To:");
			}
			{
			...
			thisLayout.setHorizontalGroup(thisLayout.createSequentialGroup()
				...
				.addGroup(thisLayout.createParallelGroup()
				    ...
				    .addGroup(thisLayout.createSequentialGroup()
				        .addGap(26)
				        .addGroup(thisLayout.createParallelGroup()
				            .addComponent(getBtnRefresh(), GroupLayout.Alignment.LEADING, GroupLayout.PREFERRED_SIZE, 85, GroupLayout.PREFERRED_SIZE)
				            .addGroup(thisLayout.createSequentialGroup()
				                .addPreferredGap(getBtnRefresh(), btnApplySelected, LayoutStyle.ComponentPlacement.INDENT)
				                .addGroup(thisLayout.createParallelGroup()
		...
		if(applyClicked == null) {
			applyClicked = new AbstractAction(">", null) {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					GridJob tempJob;
					GridUserWrapper tempUser = (GridUserWrapper)applyToComboModel.getSelectedItem();
					
					for (int i = 0; i < indicies.length; i++)
					{
						tempJob = (GridJob)availableModel.getElementAt(indicies[i]);
						tempJob.setOwner(tempUser);
		...
		if(removeClicked == null) {
			removeClicked = new AbstractAction("<", null) {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					for (int i = 0; i < indicies.length; i++)
					{
						temp = (GridJob)currentlyAppliedModel.getElementAt(indicies[i]);
						temp.setOwner(null);
					}
	...
	private void update()
	{
		...
		in = data.getAssignedJobs((GridUserWrapper)applyToComboModel.getSelectedItem());
		for (int i = 0; i < in.length; i++)
		...
		if(comboChanged == null) {
			comboChanged = new AbstractAction("", null) {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (data != null)
					{	
						Object[] in = data.getAssignedJobs((GridUserWrapper)cb.getSelectedItem());
						for (int i =0 ;i < in.length; i++)
							currentlyAppliedModel.addElement(in[i]);
					}
				}
	...
	private JButton getBtnRefresh() {
		if(btnRefresh == null) {
			...
			btnRefresh.setAction(getRefreshClick());
		}
		return btnRefresh;
	}
	
	private AbstractAction getRefreshClick() {
		if(refreshClick == null) {
			refreshClick = new AbstractAction("Refresh", null) {
				/**
				 * 
				 */
				private static final long serialVersionUID = 1L;
