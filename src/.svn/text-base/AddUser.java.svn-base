	...
	*/
	...
	private JLabel getLblUserName() {
		if(lblUserName == null) {
			...
			lblUserName.setText("New User Name");
		}
		return lblUserName;
	}
	...
	public JList getUserList() {
		if(userList == null) {
			...
			userListModel = new DefaultComboBoxModel(data.getAllUsers());
			userList = new JList();
	...
	private JButton getDeleteSelected() {
		if(deleteSelected == null) {
			...
			deleteSelected.setText("Delete Selected User");
			...
			deleteClicked = new AbstractAction("Delete Selected", null) {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					for ( int i = indicies.length-1 ; i >= 0 ; i --)
					{
						...
						if (data.deleteUser(userListModel.getElementAt(indicies[i])))
						{
							userListModel.removeElementAt(indicies[i]);
						}
						else
						{
							JOptionPane.showMessageDialog(null, "User Not deleted for some reason");
						}
		...
		if(addClicked == null) {
			addClicked = new AbstractAction("Add", null) {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					{
						if (data.addUser(txtNewName.getText()))
						{
						...
						{
							JOptionPane.showMessageDialog(null, "Add failed - does the username already exist?");
						}
					}
				}
			};
	...
	public void refresh()
	{
		...
		GridUserWrapper[] g = data.getAllUsers();
